...
| `AGENT_LLM_PROVIDERS` | Ordered LLM list | Default `openrouter,gemini`; first with a key wins |
| `OPENROUTER_API_KEY` | SMIS Agent (OpenRouter) | Used when listed first among configured providers |
| `OPENROUTER_MODEL` | OpenRouter model id | Default `openai/gpt-oss-20b:free` |
| `GEMINI_API_KEY` | SMIS Agent (Gemini) | Used when OpenRouter has no key, or listed first |
