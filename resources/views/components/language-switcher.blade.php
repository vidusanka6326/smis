...
@if ($variant === 'pills')
    <div
        {{ $attributes->class('inline-flex overflow-hidden rounded-md bg-black/20') }}
        role="group"
        aria-label="{{ __('Language') }}"
        data-test="language-switcher"
    >
        @foreach (\App\Enums\AppLocale::cases() as $locale)
            <form method="POST" action="{{ route('locale.update') }}" class="min-w-0 flex-1 border-e border-white/15 last:border-e-0">
                @csrf
                ...
                <button
                    ...
                    class="min-h-9 w-full whitespace-nowrap px-2.5 py-1.5 text-center font-display text-[11px] font-semibold tracking-wide {{ $locale === $current ? 'bg-white text-[var(--home-ink)]' : 'text-white/80 hover:bg-white/10 hover:text-white' }}"
