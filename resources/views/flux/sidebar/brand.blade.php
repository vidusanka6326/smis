...
$classes = Flux::classes()
    ->add($subtitle ? 'h-12' : 'h-10')
    ->add('min-w-0 flex items-center px-2 in-data-flux-sidebar-collapsed-desktop:w-10 in-data-flux-sidebar-collapsed-desktop:px-0 in-data-flux-sidebar-collapsed-desktop:justify-center')
...
<?php if ($name): ?>
    <a href="{{ $href }}" {{ $attributes->class([ $classes, 'gap-2' ]) }} data-flux-sidebar-brand>
        ...
        <div class="min-w-0 grid leading-tight in-data-flux-sidebar-collapsed-desktop:hidden">
            <div class="{{ $textClasses }}">{{ $name }}</div>
            <?php if ($subtitle): ?>
                <div class="truncate text-xs font-normal text-zinc-500 dark:text-zinc-400">{{ $subtitle }}</div>
            <?php endif; ?>
        </div>
    </a>
<?php else: ?>
