<x-layouts::app>
    <div class="mb-6">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>{{ __('Create Lesson') }}</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        
        <flux:heading size="xl" class="mt-4">{{ __('Create Video Lesson') }}</flux:heading>
        <flux:subheading>{{ __('Add a new video lesson and homework for your students') }}</flux:subheading>
    </div>
    ...
    <form method="POST" action="{{ route('teacher.lessons.store') }}" class="mt-6 space-y-6">
        @csrf

        <x-form.section :title="__('Lesson Details')" :description="__('Provide the video and homework instructions for this lesson.')">
            <x-form.grid>
                <flux:input name="title" :label="__('Title')" :value="old('title')" required autofocus class="sm:col-span-2" />
                
                <flux:select name="school_class_ids[]" :label="__('Classes')" multiple required>
                    @foreach ($classes as $class)
                        <flux:select.option :value="$class->id" :selected="in_array($class->id, old('school_class_ids', []))">{{ $class->code }}</flux:select.option>
                    @endforeach
                </flux:select>
                
                <flux:select name="subject_id" :label="__('Subject')" required>
                    <flux:select.option value="" disabled selected>{{ __('Select a subject') }}</flux:select.option>
                    @foreach ($subjects as $subject)
                        <flux:select.option :value="$subject->id" :selected="old('subject_id') == $subject->id">{{ $subject->name }}</flux:select.option>
                    @endforeach
                </flux:select>

                <flux:input name="youtube_url" type="url" :label="__('YouTube URL')" :value="old('youtube_url')" placeholder="https://www.youtube.com/watch?v=..." class="sm:col-span-2" />
        ...
        <div class="flex items-center justify-end gap-x-4 border-t border-gray-900/10 dark:border-gray-100/10 pt-6">
            ...
            <flux:button type="submit" variant="primary">{{ __('Save Lesson') }}</flux:button>
        </div>
    </form>
</x-layouts::app>
